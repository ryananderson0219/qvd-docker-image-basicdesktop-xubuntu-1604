...
# Disable services
RUN for i in anacron atd cron cups-browsed kerneloops ModemManager rsyslog whoopsie ;  do systemctl disable $i.service; done
# Cleanup
RUN echo "" > /etc/udev/rules.d/70-persistent-net.rules
# Currently has a bug
RUN apt-get -y remove blueman wpasupplicant
